class WorktimesController < ApplicationController
	...
	def index
		if params[:id].nil?
			...
			@worktimes = Worktime.where("user_id = ? and checkin > ? and checkin < ?", @user.id.to_s, Time.parse(params[:from]), Time.parse(params[:to]).advance(:hours => 24))
			@worktimes_paged = @worktimes.order(sort_column + ' ' + sort_direction).paginate(:page => params[:page], :per_page => 10)
			if !@worktime.nil?
